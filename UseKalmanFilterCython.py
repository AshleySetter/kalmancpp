...
data.time = np.arange(0, 30*2*np.pi/w, Ts) # 30 periods of oscillation
...
kf = kalmancpp.CKalmanFilter(N, np.array(A), np.array(P), np.array(Q), np.array(H).flatten(), np.array(x_init).flatten(), float(R))
